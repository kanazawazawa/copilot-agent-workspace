...
|-------------|-------------------|-------------|---------|
| `COPILOT_FG_TOKEN` | **Public Repositories** (all) | `Copilot Requests: read` | SDK execution (`copilot-poc.yml`, healthcheck Step 4) |
| `REPO_FG_TOKEN` | **Specific repository** only | `Metadata: read`, `Actions: read/write`, `Contents: read/write`, `Issues: read/write`, `Pull requests: read/write` | Issue creation + Copilot Coding Agent assignment (healthcheck Steps 5–6) |
...
│   │   └── copilot-healthcheck.yml  # Daily healthcheck + auto Issue + Copilot assignment
...
|-----------|-------------------|---------------|------|
| `COPILOT_FG_TOKEN` | **Public Repositories**（全公開リポジトリ） | `Copilot Requests: read` | SDK 実行（`copilot-poc.yml`、healthcheck Step 4） |
| `REPO_FG_TOKEN` | **特定リポジトリ** のみ | `Metadata: read`, `Actions: read/write`, `Contents: read/write`, `Issues: read/write`, `Pull requests: read/write` | Issue 作成 + Copilot Coding Agent アサイン（healthcheck Steps 5–6） |
...
│   │   └── copilot-healthcheck.yml  # 毎日ヘルスチェック + 自動 Issue + Copilot アサイン
